...
const DEFAULT_FOXWORK_EXTENSION_MANIFESTS = [
  {
    ...
    id: "openwork-browser",
    name: "SeeWayWork 浏览器",
    description: "使用 SeeWayWork 内置的可见浏览器完成网页操作。",
    source: { format: "openwork-builtin", origin: "builtin", trusted: true },
    icon: { src: "/openwork-mark.svg" },
    composer: { prompt: "使用 SeeWayWork 浏览器完成" },
    setup: { instructions: "SeeWayWork 浏览器已在桌面工作区中默认就绪。" },
    ...
    contributions: [
      ...
      { type: "composer-prompt", prompt: "使用 SeeWayWork 浏览器完成", location: "composer" },
    ],
  ...
  },
  {
    ...
    setup: { instructions: "添加 OpenAI API 密钥后，智能体即可通过 SeeWayWork 扩展操作生成图片资料。" },
    resources: [
  ...
  },
  {
    ...
    id: "google-workspace",
    name: "Google 工作区",
    description: "让 SeeWayWork 协助处理会议、指定云端硬盘文件和 Gmail 草稿。",
    source: { format: "openwork-builtin", origin: "builtin", trusted: true },
    icon: { simpleIconSlug: "google" },
    composer: { prompt: "使用 Google 工作区完成" },
    setup: { instructions: "连接 Google 账号后即可在 SeeWayWork 中使用日历、云端硬盘和 Gmail 草稿。" },
    resources: [
...
function serializePluginExtension(row: PluginRow, componentCounts: Record<string, number>) {
  ...
  return {
    ...
    manifest: {
      ...
      setup: {
        instructions: "Imported from a Claude-compatible plugin. SeeWayWork installs its resources into this workspace as extension components.",
      },
      lifecycle: {
