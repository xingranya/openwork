  ...
  if (error) {
    return (
      <div className="p-4 text-sm text-muted-foreground">
        {toChineseUserMessage(error, "无法解析此表格，请检查文件格式。")}
      </div>
    );
  }
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col", props.className)}>
      <div className="flex shrink-0 items-center gap-2 px-3 py-2 border-b border-border">
        <Button variant="ghost" size="xs" onClick={addRow}><Plus className="size-3" /> 添加行</Button>
        <Button variant="ghost" size="xs" onClick={addColumn}><Plus className="size-3" /> 添加列</Button>
        <div className="min-w-0 flex-1" />
        <Button variant="ghost" size="xs" onClick={discard} disabled={!isDirty || saving}>放弃</Button>
        <Button variant="default" size="xs" onClick={() => save()} disabled={!isDirty || saving}>{saving ? "正在保存" : "保存"}</Button>
      </div>
