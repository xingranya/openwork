...
The artifact preview keeps **outside-chat Markdown** readable with inline \`surface renderer\`, a fenced code block, and [SeeWayWork](https://openworklabs.com).
  ...
  const seedPdfArtifactControlAction = React.useMemo<OpenworkControlAction | null>(() => {
    ...
    return {
      ...
      execute: async () => {
        ...
        // Minimal single-page PDF that draws "SeeWayWork PDF" — base64 encoded.
        const pdfBase64 =
          ...
          <div className="flex h-10 items-center gap-1 border-b border-border/60 px-2">
            ...
            {isBrowserAvailable ? (
              <Tooltip>
                ...
                />
                <TooltipContent>新建标签页</TooltipContent>
              </Tooltip>
            ) : null}
          </div>
        </div>
...
function PanelEmpty() {
  return (
    <div className="flex h-full items-center justify-center p-4 text-center">
      <p className="text-sm text-muted-foreground">打开文件或浏览器标签页后，可在这里查看。</p>
    </div>
  );
}
